...
CREATE TABLE IF NOT EXISTS "public"."reader_library" (
    ...
    "notes" "text",
    CONSTRAINT "reader_library_acquired_from_check" CHECK ((("acquired_from")::"text" = ANY (ARRAY[('giveaway'::character varying)::"text", ('purchase'::character varying)::"text", ('gift'::character varying)::"text", ('author_direct'::character varying)::"text"]))),
...
ALTER TABLE ONLY "public"."reader_library"
    ADD CONSTRAINT "reader_library_reader_id_fkey" FOREIGN KEY ("reader_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
CREATE POLICY "Authors can view download logs for their books" ON "public"."reader_download_logs" FOR SELECT USING ((( SELECT "auth"."uid"() AS "uid") IN ( SELECT "b"."user_id"
   FROM ("public"."books" "b"
     JOIN "public"."book_files" "bf" ON (("b"."id" = "bf"."book_id")))
...
CREATE POLICY "Service role can manage analytics" ON "public"."analytics_events" USING ((( SELECT "auth"."role"() AS "role") = 'service_role'::"text"));



COMMENT ON POLICY "Service role can manage analytics" ON "public"."analytics_events" IS 'Optimized RLS policy with SELECT subquery for better performance';
...
CREATE POLICY "Users can add books to their library" ON "public"."reader_library" FOR INSERT WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "reader_id"));
...
CREATE POLICY "Users can track their reading progress" ON "public"."reading_progress" FOR INSERT WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Users can update their library" ON "public"."reader_library" FOR UPDATE USING ((( SELECT "auth"."uid"() AS "uid") = "reader_id"));
...
CREATE POLICY "Users can update their reading progress" ON "public"."reading_progress" FOR UPDATE USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Users can view their delivery methods" ON "public"."book_delivery_methods" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."books"
  WHERE (("books"."id" = "book_delivery_methods"."book_id") AND ("books"."user_id" = ( SELECT "auth"."uid"() AS "uid"))))));
...
CREATE POLICY "Users can view their own library" ON "public"."reader_library" FOR SELECT USING ((( SELECT "auth"."uid"() AS "uid") = "reader_id"));
...
CREATE POLICY "Users can view their own reading progress" ON "public"."reading_progress" FOR SELECT USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));
